  ...
  def self.search(query)
    if query
      ...
      if /^[a-zA-Z ]*$/.match(my_query)
        ...
        joins(:orders).where(:orders => {:status => "QUEUED"}).where("customers.shopify_customer_id = ? OR id = ?", "#{my_query}", "#{my_query}").order(first_name: :asc)
      end
    else
      joins(:orders).where(:orders => {:status => "QUEUED"}).order(first_name: :asc)
    end
  end
end
